...
class Controller extends BaseController {
  ...
  pariseReply = async (replyId, createBy) => {
    ...
    if (!pariseRecord) {
      ...
      await this._model.updateOne(
        { createBy, replyId },
        { status: !pariseRecord.status }
      );
    }

    let inc = !pariseRecord || !pariseRecord.status ? 1 : -1;
    let result = await Reply.findByIdAndUpdate(replyId, {
      $inc: { praiseCount: inc },
    }); // 帖子的点赞数+-1
    await Record.findOneAndUpdate({ createBy }, { $inc: { praiseCount: inc } }); // 用户的点赞数+-1
    await Record.findOneAndUpdate(
      { createBy: replyRecord.createBy },
      { $inc: { bePraiseCount: inc } }
    ); // 用户的被点赞数+-1
    return res(result);
  };
  ...
  pariseTopic = async (topicId, createBy) => {
    ...
    let inc = !pariseRecord || !pariseRecord.status ? 1 : -1;
    let result = await Topic.findByIdAndUpdate(
      topicId,
      { $inc: { praiseCount: inc } },
      { new: true }
    )
