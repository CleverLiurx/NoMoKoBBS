  ...
  this.findOne({ delFlag: false })
    .populate({
      ...
      populate: {
        path: "record",
        select:
          "beCommentCount bePraiseCount beStarCount commentCount pointCount praiseCount starCount topicCount",
      },
    })
...
schema.pre("find", function () {
  this.find({ delFlag: false })
    .populate("createBy", "_id username sex avator email")
    .populate("classFrom", "_id classname");
});
...
schema.post("findOne", async function (doc) {
  if (doc) {
    ...
    // 看自己是否点赞
    const pariseRecord = await Parise.findOne({
      createBy: doc.createBy._id,
      topicId: doc._id,
      status: true,
    });
